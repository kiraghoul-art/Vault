...
// All state lives in memory only. sessionStorage used for worker URL only.
// Tab closes = everything gone.

const WORKER_URL_KEY = 'kira_worker';

let workerUrl = sessionStorage.getItem(WORKER_URL_KEY) || '';
let cfg = {};
let shorts = [];
let vaultItems = [];
...
async function doLogin() {
  ...
  if (!username || !password) { showMsg('login-result', false, 'Enter username and password.'); return; }
  if (!workerUrl) {
    // First ever load — ask for worker URL
    const w = prompt('One-time setup: paste your Cloudflare Worker URL');
    if (!w) return;
    workerUrl = w.trim();
    sessionStorage.setItem(WORKER_URL_KEY, workerUrl);
  }

  btn.textContent = 'Verifying…'; btn.disabled = true;
  result.className = 'login-result'; result.textContent = '';

  let authRes;
  try {
    authRes = await DISCORD.post('/auth', { username, password });
  } catch (e) {
    btn.textContent = 'Enter'; btn.disabled = false;
    showMsg('login-result', false, 'Cannot reach worker. Check URL.'); return;
  }

  if (!authRes.ok) {
    btn.textContent = 'Enter'; btn.disabled = false;
    showMsg('login-result', false, 'Invalid credentials.'); return;
  }

  btn.textContent = 'Enter'; btn.disabled = false;

  if (authRes.firstTime || !authRes.config) {
    // No config yet — show first-time setup
    document.getElementById('login-screen').style.display = 'none';
    document.getElementById('setup-screen').style.display = 'flex';
    return;
  }

  // Load config and enter app
  cfg = authRes.config;
  enterApp();
}
...
document.getElementById('btn-setup-save').addEventListener('click', async () => {
  ...
  if (!links || !notes || !files || !ideas) {
    showMsg('setup-result', false, 'All channel IDs are required.'); return;
  }

  const btn = document.getElementById('btn-setup-save');
  btn.textContent = 'Saving…'; btn.disabled = true;

  const newCfg = { links, notes, files, ideas };
  try {
    const res = await DISCORD.post('/config/save', { config: newCfg });
    if (res.ok) {
      cfg = newCfg;
      document.getElementById('setup-screen').style.display = 'none';
      enterApp();
    } else {
      showMsg('setup-result', false, 'Failed to save. Check channel IDs.');
    }
  } catch (e) {
    showMsg('setup-result', false, 'Error: ' + e.message);
  }
...
async function syncFromDiscord() {
  ...
  lbl.textContent = 'Discord: syncing…';
  const types = [
  ...
  lbl.textContent = 'Discord: connected';
}
...
document.getElementById('btn-save-profile').addEventListener('click', async () => {
  ...
  showMsg('profile-result', res.ok, res.ok ? 'Profile saved to Discord.' : 'Failed to save.');
});
...
function renderShorts(filter = 'all') {
  ...
  grid.innerHTML = items.map((s, i) => `
    <div class="short-card" ${s.url ? `onclick="window.open('${esc(s.url)}','_blank')"` : ''}>
      <div class="short-thumb" style="background:${THUMB_COLORS[i%THUMB_COLORS.length]}">
        ...
        <div class="thumb-tag">${esc(s.cat)}</div>
      </div>
      <div class="short-info">
        <div class="short-title">${esc(s.title)}</div>
        <div class="short-meta">
          <span class="short-date">${esc(s.date)}</span>
...
function renderVault(items = null) {
  ...
  c.innerHTML = all.map(v => `
    <div class="vault-item" ${v.url ? `onclick="window.open('${esc(v.url)}','_blank')"` : ''}>
      <div class="vault-item-type">${TYPE_ICON[v.type]||'·'} ${esc(v.type)}</div>
      ...
      <div class="vault-item-footer">
        <div style="display:flex;gap:0.35rem;flex-wrap:wrap;">${(v.tags||[]).map(t=>`<span class="vault-tag">${esc(t)}</span>`).join('')}</div>
        <span class="vault-date">${esc(v.date)}</span>
      </div>
    </div>`).join('');
}
...
function renderTagSidebar() {
  ...
  s.innerHTML = '<div class="vault-sidebar-title" style="padding-top:0">Tags</div>' +
    tags.map(t => `<div class="vault-cat" data-tag="${esc(t)}"><span class="vault-cat-icon">◈</span> ${esc(t)}</div>`).join('');
  s.querySelectorAll('[data-tag]').forEach(el => el.addEventListener('click', () => {
